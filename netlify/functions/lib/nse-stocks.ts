 ...
 * Ported from Python nse_stocks.py
 * Each entry: [yahooSymbol, displayName, sector]
 */
...
export const NSE_STOCKS: Array<[string, string, string]> = [
  // ── LARGE CAP INDEX ──────────────────────────────────────────────────────
...
// Build lookup: yahooSymbol → { name, sector }
export const STOCK_MAP = new Map<string, { name: string; sector: string }>(
  NSE_STOCKS.map(([sym, name, sector]) => [sym, { name, sector }])
);

// All unique sectors (sorted)
export const SECTORS: string[] = [...new Set(NSE_STOCKS.map(([, , s]) => s))].sort();

// Helper: convert tuple to StockEntry
export function toStockEntry([symbol, name, sector]: [string, string, string]): StockEntry {
  return {
    symbol,
    displaySymbol: symbol.replace(".NS", "").replace("^", ""),
    name,
    sector,
  };
}
